...
Shapefiles are still among the most common formats to store and transmit spatial data, despite them being inefficient (file size and file number). Similarly you can read `geo.json`, which usually is more efficient, but less common (but we're getting there).
