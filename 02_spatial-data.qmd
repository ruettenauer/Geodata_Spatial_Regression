...
To do this, we first create a 1 km buffer around each centroid using st_buffer(). Obviously, you could also use the existing polygons (e.g. each LSOA shape) rather than such a buffer around the mid/point. We then use area-weighted interpolation from the areal package to estimate average pollution values within each buffer. This approach accounts for the proportion of each pollution grid cell that overlaps with the buffer area and provides a more streamlined alternative to manually calculating intersections and weighted averages.

In the commented section below, you also see an older version that uses `st_intersetion()` to calculate the overlap manually.
...
4) Please calculate the distance of each MSOA to the London city centre 
  a) use google maps to get lon and lat of the city centre (roughly), 
