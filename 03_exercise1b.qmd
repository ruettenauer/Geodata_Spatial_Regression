...
```{r}
# Centroids and K nearest neighbours
coords <- st_centroid(msoa.spdf)
k10.nn <- knearneigh(coords, k = 10)

### create nb object
k10.nb <- knn2nb(k10.nn)
summary(k10.nb)
...
# Extract ten nearest neighbours
j <- k10.nn$nn[i,]
...
```{r}
# MOran test
moran.test(msoa.spdf$per_nonEU, listw = km5.lw)
```
...
```{r}
loci2 <- localmoran(msoa.spdf$per_nonEU, listw = km5.lw)
