...
pkgs <- c("sf", "mapview", "spdep", "spatialreg", "tmap", 
          "ggplot2", "viridis", "viridisLite") # note: load spdep first, then spatialreg
...
## Using this method for ego-hoods
...
For a practical examples, see for instance @Haussmann.2024: we test the impact of individual deprivation and neighbourhood deprivation on the likelihood of voting for Brexit. We find that more deprived people are more likely to favour Brexit. Moreover, the effect of individual deprivation is stronger in richer areas - being deprived is more frustrating if you see a lot of well-off people living around you.

Obviously, a crucial question is the the size of the area / the units that we include in creating such a variable. Likely, we could also take into account multiple definition and then do a robustness check with varying distances [@Haussmann.2024]. 
...
focal.cent <- st_centroid(msoa.spdf[which(msoa.spdf$MSOA11CD == focal), ])
...
cover <- st_intersection(london_filtered2, buf2)

# Get centroids of all units
cent.sp <- st_centroid(msoa.spdf)


### Plot the buffers and add the neighbours as cross 
mp1_l <- 
  ggplot() +
