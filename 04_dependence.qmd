...
pkgs <- c("sf", "gstat", "mapview", "spdep", "tmap", "viridisLite") # note: load gstat first, then spdep
...
To calculate the empirical semi-variogram, we can use the package `gstat` with the function `variogram()`.

```{r}
# Variogram No2
v.no2 <- gstat::variogram(no2 ~ 1, msoa.spdf)
...
# least square fit
m.f.v.no2 <- gstat::fit.variogram(v.no2, m.no2)
```
