...
semfe.mod <- spml(formula = fm, data = Produc, 
                  ...
                  method = "eigen",       # estimation method
...
The problem here is that we only get the coefficient, but no impact calculations (as far as I know). This is not a problem for SEM-like model, but it is a problem with SAR-like models.
...
```{r, eval=FALSE}
### FE SAR model
sarfe.mod2 <- SDPDm(formula = form1, 
                    data = Produc, 
                    W = usaww,                 
